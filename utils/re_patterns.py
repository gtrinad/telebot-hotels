PATTERN_CITY = (
    r'"type":"CITY".*?"fullName":"(?P<fullName>[^"]+).*?"sourceId":"(?P<sourceId>[^"]+).*?"'
    r'coordinates":{"lat":"(?P<latitude>-?\d+\.\d+)","long":"(?P<longitude>-?\d+\.\d+)'
)

PATTERN_PROPERTY = (
    r'"Property","id":"(?P<Property_id>\d+).*?"name":"(?P<name>[^"]+).*?"distanceFromDestination":{'
...
PATTERN_SUMMARY = (
    r'"propertyRating":{".*?"rating":(?P<rating>\d\.?\d?).*?"PropertyAddress","addressLine":"('
