...
def request_hotels(chat_id: int, sort: str) -> None:
    ...
    payload: Dict = {
        ...
        "resultsSize": 200,
        "sort": sort,
        "filters": {"price": {"max": 1000000, "min": 1}},
    }
    ...
    logger.info(
        f"Чат - {chat_id}, по команде {user_command} отправлено информации о {count_hotels} отелях."
    )
...
def send_hotel_info(
    ...
    if show_photo > 0:
        ...
        if get_state(chat_id=chat_id, column="states") == "send_result":
            ...
            bot.send_message(
                chat_id=chat_id, text=hotel_info, disable_web_page_preview=True
            )
        else:
            return
