...
from utils.misc.hotels_requests import format_star_rating, handle_no_results, send_hotel_info, finish_search
...
def request_hotels_custom(chat_id: int) -> None:
    ...
    destination_id, check_in, check_out, hotels_count, user_command, show_photo, price_min, price_max = (
        get_current_requests(chat_id=chat_id, column=column)
        for column in
        ["destination_id", "check_in", "check_out", "hotels_count", "current_command", "images_count", "price_min", "price_max"]
    )
