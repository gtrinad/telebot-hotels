...
def calendar_call(call: CallbackQuery, sep, text_choice: str, markup_day, markup_cancel, edit_column: str) -> None:
    """
    ...
    if action == "DAY":
        ...
        bot.send_message(chat_id=chat_id, text=f"{text_choice} {date_str}", reply_markup=markup_day)
        if edit_column in ("check_in", "check_out"):
            set_current_requests(chat_id=chat_id, **{f"{edit_column}": date_str})
    elif action == "CANCEL":
        bot.send_message(chat_id=chat_id, text="Хотите завершить?", reply_markup=markup_cancel)
