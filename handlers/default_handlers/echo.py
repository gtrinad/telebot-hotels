...
def bot_echo(message: Message) -> None:
    ...
    logger.info(
        f"В чате - {message.chat.id} пользователь {message.from_user.first_name} набрал текст: {message.text}"
    )
...
def message_reply(message: Message) -> None:
    ...
    logger.info(
        f"В чате - {message.chat.id} пользователь {message.from_user.first_name} отправил: {message.content_type}"
    )
    bot.reply_to(message=message, text="Я тебя не понимаю. Напиши /help.")
