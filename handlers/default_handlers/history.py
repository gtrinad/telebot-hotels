...
def send_history(message: Message) -> None:
    ...
    logger.info(
        f"В чате - {chat_id} пользователь {message.from_user.first_name} запустил команду /history"
    )
    ...
    if not data:
        ...
        bot.send_message(
            chat_id=chat_id,
            text="Выберите историю поиска, которую показать:",
            reply_markup=markup,
        )
...
@logger.catch
def send_character_page(
    message: Message, page: int = 1, command: str = None, city_id: int = None
):
    """
    ...
    logger.info(
        f"В чате - {chat_id} пользователь {message.from_user.first_name} получил историю отелей пагинацией."
    )

    command_pickle, city_id_pickle = (
        get_pagination(message.id) if not command else (command, city_id)
    )
    data: Dict = get_pickle(chat_id)
    hotels_pages: List = data[command_pickle][city_id_pickle][1:]

    paginator: InlineKeyboardPaginator = InlineKeyboardPaginator(
        page_count=len(hotels_pages), current_page=page, data_pattern="character#{page}"
    )
