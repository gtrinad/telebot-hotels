...
@bot.message_handler(
    func=lambda message: "hotels_number"
...
def hotels_number_choice(message: Message) -> None:
    ...
    logger.info(
        f"В чате - {chat_id} пользователь {message.from_user.first_name} выбирает кол-во показываемых отелей."
    )
    ...
    current_id_message = bot.send_message(
        chat_id=chat_id,
        text="Показывать фотографии отелей?",
        reply_markup=markup_choice_photo(),
    )
